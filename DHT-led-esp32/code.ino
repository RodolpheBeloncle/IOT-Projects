...
void control_LEDs() {
  ...
  if (myObject.hasOwnProperty("led")) {
    ...
    Serial.println(myObject["led"][0]["status"]);
  }
  ...
  // compare two strings values => if led status equal to current value return 0
  if (strcmp(myObject["led"][0]["status"], "ON") == 0) {
    digitalWrite(LED_01, HIGH);
    Serial.println("LED 01 ON");
  }
  if (strcmp(myObject["led"][0]["status"], "OFF") == 0) {
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // postData = "id=1";
    postData += "temp=" + String(send_Temp);
    postData += "&hum=" + String(send_Humd);
    // postData += "&status_read_sensor_dht11=" + send_Status_Read_DHT11;
    // postData += "&status=" + LED_01_State;
    // postData = "&id=2";
    // postData += "&status=" + LED_02_State;
    payload = "";
    ...
    http.begin("=" + String(send_Temp) + "&hum=" + String(send_Humd));  //--> Specify request destination
    http.addHeader("Content-Type", "application/x-www-form-urlencoded");                //--> Specify content-type header
